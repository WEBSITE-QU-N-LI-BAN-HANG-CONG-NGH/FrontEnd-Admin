...
const dashboardService = {
  ...
  getRevenueDistribution: () => api.get('/admin/dashboard/revenue-distribution'),
};

const productService = {
  getAllProducts: () => api.get('/admin/products/all'),
  createProduct: (productData) => api.post('/admin/products/create', productData),
  updateProduct: (productId, productData) => api.put(`/admin/products/${productId}/update`, productData),
  deleteProduct: (productId) => api.delete(`/admin/products/${productId}/delete`),
  getTopSellingProducts: (limit = 10) => api.get(`/admin/products/top-selling?limit=${limit}`),
...
const orderService = {
  ...
  getOrderStats: (startDate, endDate) => 
    api.get(`/admin/orders/stats?startDate=${startDate || ''}&endDate=${endDate || ''}`),
};

const reportService = {
  getProductReports: (startDate, endDate) => 
    api.get(`/admin/reports/products?startDate=${startDate || ''}&endDate=${endDate || ''}`),
  getProductStats: () => api.get('/admin/reports/products/stats'),
};

const categoryService = {
  getAllCategories: () => api.get('/admin/categories/'),
  updateCategory: (categoryData) => api.put('/admin/categories/update', categoryData),
